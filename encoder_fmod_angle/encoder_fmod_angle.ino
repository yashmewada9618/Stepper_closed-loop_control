...
/*float dist_short(float target) {

  float signedDiff = 0.0;
  float raw_diff = degAngle > target ? degAngle - target : target - degAngle;
  float mod_diff = fmod(raw_diff, 360); //equates rollover values. E.g 0 == 360 degrees in circle

  if (mod_diff > 180 ) {
    //There is a shorter path in opposite direction
    signedDiff = (360 - mod_diff);
    if (target > degAngle) signedDiff = signedDiff * -1;
  }
  else {
    signedDiff = mod_diff;
    if (degAngle > target) signedDiff = signedDiff * -1;
  }
  while (1) {
    ReadRawAngle();
    if (signedDiff <= 1.8 && signedDiff >= 0.0) {
...
void shortest_dist(word ang = 0) {
  while (1) {
    ...
    if (degAngle >= (ang - 0.9) && degAngle <= (ang + 0.9)) {
      ...
      if (fmod((ang - degAngle + 360), 360) < 180) {
        Serial.printf("motor homing in progress ++ \n");
        init_steps = init_steps + 1;
        stepper.moveTo(init_steps);
